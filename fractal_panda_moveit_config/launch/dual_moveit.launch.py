...
def generate_launch_description():
    ...
    franka_semantic_xacro_file = os.path.join(get_package_share_directory('fractal_panda_moveit_config'),
                                              'srdf',
                                              'dual_panda_arm.srdf.xacro')
    robot_description_semantic_config = Command(
        [FindExecutable(name='xacro'), ' ', franka_semantic_xacro_file, ' hand_1:=', load_gripper_1, ' hand_2:=', load_gripper_2, ' arm_id_1:=', arm_id_1, ' arm_id_2:=', arm_id_2, ' parent_frame:=', parent_frame]
    )
    ...
    moveit_simple_controllers_yaml = load_yaml(
        'fractal_panda_moveit_config', 'config/dual_panda_controllers.yaml'
    )
    moveit_controllers = {
    ...
    # RViz
    rviz_base = os.path.join(get_package_share_directory('fractal_panda_moveit_config'), 'rviz')
    rviz_full_config = os.path.join(rviz_base, 'dual_moveit.rviz')
    ...
    ros2_controllers_path = os.path.join(
        get_package_share_directory('fractal_panda_moveit_config'),
        'config',
