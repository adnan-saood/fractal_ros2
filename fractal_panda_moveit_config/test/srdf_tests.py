...
panda_xacro_file_name = path.join(get_package_share_directory('fractal_panda_moveit_config'), 'srdf',
                                  'panda_arm.srdf.xacro')


def test_load():
    urdf = xacro.process_file(panda_xacro_file_name).toxml()
    assert urdf.find('name="fractal_panda"') != -1
    assert urdf.find('tip_link="omega_contact_tip"') != -1
