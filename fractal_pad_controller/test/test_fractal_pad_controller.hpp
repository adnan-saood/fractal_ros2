...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_FRACTAL_PAD_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_FRACTAL_PAD_CONTROLLER_HPP_
...
#include "fractal_pad_controller/fractal_pad_controller.hpp"
...
using ControllerStateMsg = fractal_pad_controller::FractalPadController::ControllerStateMsg;
using ControllerReferenceMsg = fractal_pad_controller::FractalPadController::ControllerReferenceMsg;
using ControllerModeSrvType = fractal_pad_controller::FractalPadController::ControllerModeSrvType;
...
class TestableFractalPadController : public fractal_pad_controller::FractalPadController
{
  FRIEND_TEST(FractalPadControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(FractalPadControllerTest, activate_success);
  FRIEND_TEST(FractalPadControllerTest, reactivate_success);
  FRIEND_TEST(FractalPadControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(FractalPadControllerTest, test_update_logic_fast);
  FRIEND_TEST(FractalPadControllerTest, test_update_logic_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = fractal_pad_controller::FractalPadController::on_configure(previous_state);
...
class FractalPadControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_fractal_pad_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_fractal_pad_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_fractal_pad_controller")
  {
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_fractal_pad_controller/state", 10, subs_callback);
...
protected:
  ...
  std::unique_ptr<TestableFractalPadController> controller_;
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_FRACTAL_PAD_CONTROLLER_HPP_
