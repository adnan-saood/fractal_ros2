...
2. Install the package by calling `colcon build --symlink-install --packages-select fractal_panda_description`
3. (Re-)Source environment `source install/setup.bash`


> **NOTE:** If you use [RosTeamWorkspace (RTW)](https://rtw.stoglrobotics.de) than instead of the previous three steps, use `cb fractal_panda_description` command.

Now, launch description test:
```
ros2 launch fractal_panda_description view_fractal_panda.launch.xml
```
or
```
ros2 launch fractal_panda_description view_fractal_panda.launch.py
```
